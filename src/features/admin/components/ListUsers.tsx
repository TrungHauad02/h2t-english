    ...
    <Box>
      ...
      >
        <Table>
          <TableHead>
            <TableRow
              sx={{ bgcolor: isDarkMode ? color.emerald900 : color.emerald100 }}
            >
              ...
              <React.Fragment key={user.id}>
                <TableRow>
                  <TableCell>
                    <IconButton
                      size="small"
                      onClick={() => hooks.toggleRow(user.id)}
                    >
                      {hooks.openRows[user.id] ? (
                        <KeyboardArrowUp />
                      ) : (
                        <KeyboardArrowDown />
                      )}
                  ...
                  <TableCell sx={{ textAlign: "center" }}>
                    {(user.role === RolesEnum.TEACHER ||
                      ...
                      <IconButton
                        size="small"
                        onClick={() => hooks.handleEdit(user.name)}
                      >
                        <Edit />
                      </IconButton>
                    )}
                    <IconButton
                      size="small"
                      onClick={() => hooks.handleChangeStatus(user)}
                    >
                      <SwapHoriz />
                    </IconButton>
                    <IconButton
                      size="small"
                      onClick={() => hooks.handleRemove(user)}
                    >
                      <Delete />
                    </IconButton>
                  </TableCell>
                ...
                <TableRow
                  sx={{ bgcolor: isDarkMode ? color.teal900 : color.teal100 }}
                >
                  <TableCell
                    colSpan={7}
                    sx={{ padding: 0, paddingLeft: { xs: 1, md: 14 } }}
                  >
