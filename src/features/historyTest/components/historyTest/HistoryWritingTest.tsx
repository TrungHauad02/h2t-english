            ...
            <Box key={title} sx={{ mb: 4 }}>
              ...
              >
                {items.map((item, index) => {
                  const isPanelExpanded = expandedAccordions[`panel-${item.id}`] || false;
                  const matchingQuestion = allQuestions.find(q => q.id === item.id);
                  ...
                  return (
                    <Accordion 
                      ...
                      elevation={0}
                      sx={{
                        borderBottom: index < items.length - 1 ? `1px solid ${isDarkMode ? color.gray700 : color.gray200}` : 'none',
                        '&:before': {
                          display: 'none',
                        },
                      ...
                      >
                        ...
                        {item.score !== undefined && (
                          <Chip 
                            ...
                            label={`Score: ${item.score.toFixed(1)}/${maxScorePerQuestion.toFixed(1)}`}
                            size="small"
                            sx={{
                              bgcolor: item.score > 0 
                                ? `${getScoreColor(item.score, maxScorePerQuestion)}20` 
                                : isDarkMode ? 'rgba(239, 68, 68, 0.2)' : 'rgba(239, 68, 68, 0.1)',
                              color: item.score > 0 ? getScoreColor(item.score, maxScorePerQuestion) : isDarkMode ? color.red400 : color.red600,
                              fontSize: '0.75rem',
                              fontWeight: 500,
                              borderRadius: '16px'
                            }}
                                      ...
                                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                                        <Box
                                          sx={{
                                            ...
                                            justifyContent: 'center',
                                            borderRadius: '50%',
                                            bgcolor: `${getScoreColor(item.score, maxScorePerQuestion)}20`,
                                            border: `2px solid ${getScoreColor(item.score, maxScorePerQuestion)}`,
                                            flexShrink: 0
                                          }}
                                        >
                                          <Typography 
                                            ...
                                            sx={{ 
                                              fontWeight: 600,
                                              color: getScoreColor(item.score, maxScorePerQuestion)
                                            }}
                                          >
                                        ...
                                        <Box>
                                          ...
                                          >
                                            {getScoreLabel(item.score, maxScorePerQuestion)}
                                          </Typography>
                                          ...
                                          >
                                            Score out of {maxScorePerQuestion.toFixed(1)} points (Total writing section: {totalMaxScore} points)
                                          </Typography>
                                        </Box>
                                      </Box>
                                    </Box>
