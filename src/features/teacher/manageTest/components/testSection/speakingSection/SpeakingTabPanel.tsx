...
interface SpeakingTabPanelProps {
  speaking: TestSpeaking;
  index: number;
  isSelected: boolean;
  onClick: () => void;
  isEditMode: boolean;
...
export default function SpeakingTabPanel({
  speaking,
  index,
  isSelected,
  onClick,
  ...
  return (
    ...
    >
      ...
      >
        <StatusBadge isActive={!!speaking.status} size="small" />
      </Box>
        ...
        {isEditMode && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, mt: 0.5 }}>
            <Tooltip title="Move up">
              ...
              <Tooltip title={speaking.status ? "Set inactive" : "Set active"}>
              <Box
                onClick={(e) => {
                  e.stopPropagation();
                  if (onToggleStatus) {
                    onToggleStatus(speaking.id);
                  }
                }}
                sx={{
                  display: 'flex',
                  ...
                  height: 28,
                  borderRadius: '50%',
                  backgroundColor: speaking.status 
              ...
              <Box
                ...
                sx={{
                  ...
                  backgroundColor: isDarkMode 
                    ...
                    : 'rgba(0, 0, 0, 0.05)',
                  color: isDarkMode ? color.gray300 : color.gray600,
                  opacity: index === totalSpeakings - 1 ? 0.5 : 1,
                  pointerEvents: index === totalSpeakings - 1 ? 'none' : 'auto',
                  cursor: index === totalSpeakings - 1 ? 'default' : 'pointer',
                  '&:hover': index !== totalSpeakings - 1 ? {
                    backgroundColor: isDarkMode 
                      ? 'rgba(255, 255, 255, 0.2)' 
                      : 'rgba(0, 0, 0, 0.1)',
                    transform: 'scale(1.1)',
