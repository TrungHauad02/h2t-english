...
interface WritingTabPanelProps {
  writing: TestWriting;
  index: number;
  isSelected: boolean;
  onClick: () => void;
...
export default function WritingTabPanel({
  writing,
  index,
  isSelected,
  onClick,
  ...
  return (
    ...
    >
      {/* Status Badge - Positioned at the top left corner */}
      <Box 
        sx={{
          position: 'absolute',
          top: 8,
      ...
      >
        <StatusBadge isActive={!!writing.status} size="small" />
      </Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            ...
            {writing.minWords && writing.maxWords && (
              <Typography 
                variant="caption" 
                sx={{ 
                  display: 'block',
              ...
              >
                {writing.minWords} - {writing.maxWords} words
              </Typography>
            )}
          </Box>
        </Box>
        ...
        {isEditMode && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5, mt: 0.5 }}>
            ...
            <Tooltip title={writing.status ? "Set inactive" : "Set active"}>
              <Box
                onClick={(e) => {
                  e.stopPropagation();
                  if (onToggleStatus) {
                    onToggleStatus(writing.id);
                  }
                }}
                sx={{
                  display: 'flex',
              ...
              <Box
                ...
                sx={{
                  ...
                  backgroundColor: isDarkMode 
                    ...
                    : 'rgba(0, 0, 0, 0.05)',
                  color: isDarkMode ? color.gray300 : color.gray600,
                  opacity: index === totalWritings - 1 ? 0.5 : 1,
                  pointerEvents: index === totalWritings - 1 ? 'none' : 'auto',
                  cursor: index === totalWritings - 1 ? 'default' : 'pointer',
                  '&:hover': index !== totalWritings - 1 ? {
                    backgroundColor: isDarkMode 
                      ? 'rgba(255, 255, 255, 0.2)' 
                      : 'rgba(0, 0, 0, 0.1)',
                    transform: 'scale(1.1)',
